...
import com.leviathanstudio.lib.common.util.ByteUtil;
...
public abstract class TypeRW<T> implements INbtSerializer<T>, IStreamSerializer<T>, IStringSerializer<T>
{
    ...
    public static final TypeRW<byte[]> BYTE_ARRAY = new TypeRW<byte[]>()
    {
        ...
        public void writeToStream(byte[] o, DataOutput output) throws IOException
        {
            ByteUtil.writeVLI(output, o.length);
            output.write(o);
        }
        ...
        public byte[] readFromStream(DataInput input) throws IOException
        {
            int lenght = ByteUtil.readVLI(input);
    ...
    public static final Map<Class<?>, TypeRW<?>> UNIVERSAL_SERIALIZERS = ImmutableMap.<Class<?>, TypeRW<?>>builder().put(Integer.class, INTEGER).put(int.class, INTEGER).put(Boolean.class, BOOL).put(boolean.class, BOOL).put(Byte.class, BYTE).put(byte.class, BYTE).put(Double.class, DOUBLE).put(double.class, DOUBLE).put(Float.class, FLOAT).put(float.class, FLOAT).put(Long.class, LONG).put(long.class, LONG).put(Short.class, SHORT).put(short.class, SHORT).put(String.class, STRING).build();
        
    public static final Map<Class<?>, INbtSerializer<?>> NBT_SERIALIZERS = ImmutableMap.<Class<?>, INbtSerializer<?>>builder().putAll(UNIVERSAL_SERIALIZERS).build();
    
    public static final Map<Class<?>, IStringSerializer<?>> STRING_SERIALIZERS = ImmutableMap.<Class<?>, IStringSerializer<?>>builder().putAll(UNIVERSAL_SERIALIZERS).build();
}
